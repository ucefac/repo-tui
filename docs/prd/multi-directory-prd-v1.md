...
3. 导航到目标仓库目录（非 Git 仓库将显示警告，禁止确认）
4. 确认时检查是否为有效 Git 仓库
...
| AC-5 | 状态栏显示主目录路径 | P0 | 视觉检查 |
| AC-6 | 目录选择器支持两种模式 | P0 | 手动测试 |
| AC-7 | 移除主目录不影响手动添加的仓库 | P0 | 手动测试 |
| AC-8 | 支持搜索过滤主目录 | P1 | 手动测试 |
| AC-9 | 重复主目录添加时显示错误提示 | P0 | 手动测试：添加已存在目录时显示"目录已存在" |
| AC-10 | 非 Git 仓库选择时显示警告 | P0 | 手动测试：SingleRepo 模式选择非 Git 目录显示警告 |
